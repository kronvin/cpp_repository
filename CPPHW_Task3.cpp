...
private: 
	...
	void Destroy()
	{
		cout << "Building " << ID << " was destroyed!";
	};
...
public:
	...
	void Aging(int Years)                                                                                                      // Function to age the building
	{
		CurrentAge += Years;
	...
	bool IsDestroyed() const                                                                                                    // Check if the building should be destroyed
	{
	...
	while (true)
	{
		...
		else if (command == 3)
		{
			...
			if (foundbuilding != nullptr)
			{
				...
				foundbuilding->Aging(Years);

				if (foundbuilding->IsDestroyed())
				{
					Buildings.erase(remove_if(Buildings.begin(), Buildings.end(),
						[SearchID](const Building& b) { return b.GetID() == SearchID; }), Buildings.end());

					cout << "Building with ID " << SearchID << " has been removed from the system." << endl;
					break;
				}
				else
				{
					foundbuilding->PrintInfo();
				};
			}
			else
			{
